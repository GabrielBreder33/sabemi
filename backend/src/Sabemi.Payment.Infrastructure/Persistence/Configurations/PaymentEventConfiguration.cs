...
public sealed class PaymentEventConfiguration : IEntityTypeConfiguration<PaymentEvent>
{
    public void Configure(EntityTypeBuilder<PaymentEvent> builder)
    {
        ...
        builder.Property(paymentEvent => paymentEvent.TransactionId).HasMaxLength(100).IsRequired();
        builder.Property(paymentEvent => paymentEvent.ContractId).HasMaxLength(100).IsRequired();
        builder.Property(paymentEvent => paymentEvent.Amount).HasPrecision(18, 2);
        builder.Property(paymentEvent => paymentEvent.PaymentStatus).HasConversion<string>().HasMaxLength(30);
        builder.Property(paymentEvent => paymentEvent.ProcessingStatus).HasConversion<string>().HasMaxLength(30);
        builder.Property(paymentEvent => paymentEvent.RawPayload).HasColumnType("jsonb").IsRequired();
